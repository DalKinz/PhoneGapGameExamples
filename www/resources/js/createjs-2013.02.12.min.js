...
(function () {
    ...
    a.removeEventListener = function (b, g) {
        ...
        if (d) {
            var a = d[b];
            if (a) for (var c = 0, f = a.length; c < f; c++) if (a[c] == g) {
                1 == f ? delete d[b] : a.splice(c, 1);
                break
            }
        }
    };
    ...
    a.dispatchEvent = function (b, g) {
        ...
        if (b && a) {
            ...
            a = a[b.type];
            if (!a) return d;
            for (var a = a.slice(), c = 0, f = a.length; c < f; c++) {
                var h = a[c];
                h instanceof Function ? d = d || h.apply(null, [b]) : h.handleEvent &&
                    (d = d || h.handleEvent(b))
            }
        }
        return !!d
...
(function () {
    ...
    c._tick = function () {
        ...
        c._lastTime = b;
        for (var a = c._pauseable, e = c._listeners.slice(), f = e ? e.length : 0, h = 0; h < f; h++) {
            var k = e[h];
            null == k || d && a[h] || (k.tick ? k.tick(g, d) : k instanceof Function && k(g, d))
        }
        c.dispatchEvent({
            type: "tick",
            paused: d,
...
this.createjs = this.createjs || {};
(function () {
    var c = function (b, g, d, a, c, f, h, k, m) {
        this.initialize(b, g, d, a, c, f, h, k, m)
    }, a = c.prototype;
    a.stageX = 0;
    a.stageY = 0;
    a.rawX = 0;
    ...
    a.initialize = function (b, g, d, a, c, f, h, k, m) {
        this.type = b;
        this.stageX = g;
        this.stageY =
            d;
...
this.createjs = this.createjs || {};
(function () {
    var c = function (b, g, d, a, c, f) {
        this.initialize(b, g, d, a, c, f)
    }, a = c.prototype;
    c.identity = null;
    ...
    a.initialize = function (b, g, d, a, c, f) {
        null != b && (this.a = b);
        this.b = g || 0;
        this.c = d || 0;
        null != a && (this.d = a);
    ...
    a.prepend = function (b, g, d, a, c, f) {
        var h = this.tx;
        if (1 != b || 0 != g || 0 != d || 1 != a) {
            var k = this.a,
                m = this.c;
        ...
        this.ty = h * g + this.ty * a + f;
        return this
    };
    a.append = function (b, g, d, a, c, f) {
        var h = this.a,
            k = this.b,
            m = this.c,
            p = this.d;
        ...
        this.c = d * h + a * m;
        this.d = d * k + a * p;
        this.tx = c * h + f * m + this.tx;
        this.ty = c * k + f * p + this.ty;
        return this
    };
    ...
    a.prependTransform = function (b, g, d, a, e, f, h, k, m) {
        if (e % 360) {
            var p = e * c.DEG_TO_RAD;
            e = Math.cos(p);
            p = Math.sin(p)
        } else e = 1, p = 0; if (k || m) this.tx -= k, this.ty -= m;
        f || h ? (f *= c.DEG_TO_RAD, h *= c.DEG_TO_RAD, this.prepend(e * d, p * d, -p * a, e * a, 0, 0), this.prepend(Math.cos(h), Math.sin(h), -Math.sin(f), Math.cos(f), b, g)) : this.prepend(e * d, p * d, -p * a, e * a, b, g);
        return this
    };
    a.appendTransform = function (b, g, d, a, e, f, h, k, m) {
        if (e % 360) {
            var p = e * c.DEG_TO_RAD;
            e = Math.cos(p);
            p = Math.sin(p)
        ...
        f ||
            h ? (f *= c.DEG_TO_RAD, h *= c.DEG_TO_RAD, this.append(Math.cos(h), Math.sin(h), -Math.sin(f), Math.cos(f), b, g), this.append(e * d, p * d, -p * a, e * a, 0, 0)) : this.append(e * d, p * d, -p * a, e * a, b, g);
        if (k || m) this.tx -= k * this.a + m * this.c, this.ty -= k * this.b + m * this.d;
        return this
    };
    a.rotate = function (b) {
        var g = Math.cos(b);
    ...
    a.decompose = function (b) {
        ...
        g == d ? (b.rotation = d / c.DEG_TO_RAD, 0 > this.a && 0 <= this.d && (b.rotation += 0 >= b.rotation ? 180 : -180), b.skewX = b.skewY = 0) : (b.skewX = g / c.DEG_TO_RAD,
            b.skewY = d / c.DEG_TO_RAD);
        return b
    };
    a.reinitialize = function (b, g, d, a, c, f, h, k, m) {
        this.initialize(b, g, d, a, c, f);
        this.alpha = h || 1;
        this.shadow = k;
...
this.createjs = this.createjs || {};
(function () {
    var c = function (b, g, d, a, c, f, h) {
        this.initialize(b, g, d, a, c, f, h)
    }, a = c.prototype;
    a.target = null;
    a.overLabel = null;
    a.outLabel = null;
    ...
    a._isPressed = !1;
    a._isOver = !1;
    a.initialize = function (b, g, d, a, c, f, h) {
        b.addEventListener && (this.target = b, b.cursor = "pointer", this.overLabel = null == d ? "over" : d, this.outLabel = null == g ? "out" : g, this.downLabel = null == a ? "down" : a, this.play = c, this.setEnabled(!0), this.handleEvent({}), f && (h && (f.actionsEnabled = !1, f.gotoAndStop && f.gotoAndStop(h)), b.hitArea = f))
    };
    ...
    a.handleEvent = function (b) {
        var g = this.target,
            d = b.type;
        "mousedown" == d ? (b.addEventListener("mouseup", this), this._isPressed = !0, b = this.downLabel) : "mouseup" == d ? (this._isPressed = !1, b =
            this._isOver ? this.overLabel : this.outLabel) : "mouseover" == d ? (this._isOver = !0, b = this._isPressed ? this.downLabel : this.overLabel) : (this._isOver = !1, b = this._isPressed ? this.overLabel : this.outLabel);
    ...
    a.initialize = function (b) {
        ...
        if (null != b) {
            ...
            if (null != b.frames) if (b.frames instanceof Array) {
                ...
                a = b.frames;
                g = 0;
                for (d = a.length; g < d; g++) f = a[g], this._frames.push({
                    image: this._images[f[4] ? f[4] : 0],
                    rect: new createjs.Rectangle(f[0], f[1], f[2], f[3]),
                    regX: f[5] || 0,
                    regY: f[6] || 0
                })
            } else d = b.frames, this._frameWidth =
                    d.width, this._frameHeight = d.height, this._regX = d.regX || 0, this._regY = d.regY || 0, this._numFrames = d.count, 0 == this._loadCount && this._calculateFrames();
            if (null != (d = b.animations)) {
                ...
                for (var h in d) {
                    ...
                    f = d[h];
                    if ("number" == typeof f) a = b.frames = [f];
                    else if (f instanceof Array) if (1 == f.length) b.frames = [f[0]];
                    else {
                        b.frequency = f[3];
                        b.next = f[2];
                        a = b.frames = [];
                        for (g = f[0]; g <= f[1]; g++) a.push(g)
                    } else b.frequency = f.frequency, b.next = f.next, g = f.frames, a = b.frames = "number" == typeof g ? [g] :
                            g.slice(0);
    ...
    a._calculateFrames = function () {
        if (!(this._frames || 0 == this._frameWidth)) {
            ...
            for (var b = 0, g = this._frameWidth, d = this._frameHeight, a = 0, c = this._images; a < c.length; a++) {
                for (var f = c[a], h = (f.width + 1) / g | 0, k = (f.height + 1) / d | 0, k = 0 < this._numFrames ? Math.min(this._numFrames - b, h * k) : h * k, m = 0; m < k; m++) this._frames.push({
                    image: f,
                    rect: new createjs.Rectangle(m % h * g, (m / h | 0) * d, g, d),
                    regX: this._regX,
                    regY: this._regY
                });
                b += k
...
(function () {
    ...
    b.drawAsPath = function (b) {
        this._dirty && this._updateInstructions();
        for (var d, a = this._instructions, c = 0, f = a.length; c < f; c++) ((d = a[c]).path || 0 == c) && d.exec(b)
    };
    ...
    b.arcTo = function (b, d, a, e, f) {
        this._dirty = this._active = !0;
        this._activeInstructions.push(new c(this._ctx.arcTo, [b, d, a, e, f]));
        return this
    };
    b.arc = function (b, d, a, e, f, h) {
        this._dirty = this._active = !0;
        null == h && (h = !1);
        this._activeInstructions.push(new c(this._ctx.arc, [b, d, a, e, f, h]));
        return this
    };
    b.quadraticCurveTo = function (b, d, a, e) {
        this._dirty = this._active = !0;
        this._activeInstructions.push(new c(this._ctx.quadraticCurveTo, [b, d, a, e]));
        return this
    };
    b.bezierCurveTo = function (b, d, a, e, f, h) {
        this._dirty = this._active = !0;
        this._activeInstructions.push(new c(this._ctx.bezierCurveTo, [b, d, a, e, f, h]));
        return this
    };
    b.rect = function (b,
        d, a, e) {
        this._dirty = this._active = !0;
        this._activeInstructions.push(new c(this._ctx.rect, [b, d, a, e]));
        return this
    };
    ...
    b.beginLinearGradientFill = function (b, d, j, e, f, h) {
        this._active && this._newPath();
        j = this._ctx.createLinearGradient(j, e, f, h);
        e = 0;
        for (f = b.length; e < f; e++) j.addColorStop(d[e], b[e]);
        this._fillInstructions = [new c(this._setProp, ["fillStyle", j], !1), a.fillCmd];
        return this
    };
    b.beginRadialGradientFill = function (b, d, j, e, f, h, k, m) {
        this._active && this._newPath();
        j = this._ctx.createRadialGradient(j, e, f, h, k, m);
        e = 0;
        for (f = b.length; e <
            f; e++) j.addColorStop(d[e], b[e]);
        this._fillInstructions = [new c(this._setProp, ["fillStyle", j], !1), a.fillCmd];
        return this
    };
    ...
    b.setStrokeStyle = function (b, d, j, e, f) {
        this._active && this._newPath();
        this._strokeStyleInstructions = [new c(this._setProp, ["lineWidth", null == b ? "1" : b], !1), new c(this._setProp, ["lineCap", null == d ? "butt" : isNaN(d) ? d : a.STROKE_CAPS_MAP[d]], !1), new c(this._setProp, ["lineJoin", null == j ? "miter" : isNaN(j) ? j : a.STROKE_JOINTS_MAP[j]], !1), new c(this._setProp, ["miterLimit", null == e ? "10" : e], !1)];
    ...
    b.beginLinearGradientStroke = function (b, d, a, e, f, h) {
        this._active && this._newPath();
        a = this._ctx.createLinearGradient(a, e, f, h);
        e = 0;
        for (f = b.length; e < f; e++) a.addColorStop(d[e], b[e]);
        this._strokeInstructions = [new c(this._setProp, ["strokeStyle", a], !1)];
        return this
    };
    b.beginRadialGradientStroke = function (b, d, a, e, f, h, k, m) {
        this._active && this._newPath();
        a = this._ctx.createRadialGradient(a, e, f, h, k, m);
        e = 0;
        for (f = b.length; e < f; e++) a.addColorStop(d[e], b[e]);
        this._strokeInstructions = [new c(this._setProp, ["strokeStyle", a], !1)];
        return this
    ...
    b.drawRect = b.rect;
    b.drawRoundRect = function (b, d, a, c, f) {
        this.drawRoundRectComplex(b, d, a, c, f, f, f, f);
        return this
    };
    b.drawRoundRectComplex = function (b, d, a, e, f, h, k, m) {
        var p = (a < e ? a : e) /
            2,
            q = 0,
            s = 0,
        ...
        var p = this._ctx.arcTo,
            r = this._ctx.lineTo;
        this._activeInstructions.push(new c(this._ctx.moveTo, [b + a - h, d]), new c(p, [b + a + h * s, d - h * s, b + a, d + h, h]), new c(r, [b + a, d + e - k]), new c(p, [b + a + k * t, d + e + k * t, b + a - k, d + e, k]), new c(r, [b + m, d + e]), new c(p, [b - m * u, d + e + m * u, b, d + e - m, m]), new c(r, [b, d + f]), new c(p, [b - f * q, d - f * q, b + f, d, f]), new c(this._ctx.closePath));
        return this
    };
    ...
    b.drawEllipse = function (b, d, a, e) {
        this._dirty = this._active = !0;
        var f = 0.5522848 * (a / 2),
            h = 0.5522848 * (e / 2),
            k = b + a,
        ...
        a = b + a / 2;
        e = d + e / 2;
        this._activeInstructions.push(new c(this._ctx.moveTo, [b, e]), new c(this._ctx.bezierCurveTo, [b, e - h, a - f, d, a, d]), new c(this._ctx.bezierCurveTo, [a + f, d, k, e - h, k, e]), new c(this._ctx.bezierCurveTo, [k, e + h, a + f, m, a, m]), new c(this._ctx.bezierCurveTo, [a - f, m, b, e + h, b, e]));
        return this
    };
    b.drawPolyStar = function (b, d, a, e, f, h) {
        this._dirty =
            this._active = !0;
        null == f && (f = 0);
        f = 1 - f;
        ...
        for (var m = 0; m < e; m++) h += k, 1 != f && this._activeInstructions.push(new c(this._ctx.lineTo, [b + Math.cos(h) * a * f, d + Math.sin(h) * a * f])), h += k, this._activeInstructions.push(new c(this._ctx.lineTo, [b + Math.cos(h) * a, d + Math.sin(h) * a]));
        return this
    };
    b.decodePath = function (b) {
        for (var d = [this.moveTo, this.lineTo, this.quadraticCurveTo, this.bezierCurveTo,
            this.closePath
        ], c = [2, 2, 4, 6, 0], e = 0, f = b.length, h = [], k = 0, m = 0, p = a.BASE_64; e < f;) {
            var q = b.charAt(e),
                s = p[q],
                t = s >> 3,
                u = d[t];
            if (!u || s & 3) throw "bad path data (@" + e + "): " + q;
            q = c[t];
            t || (k = m = 0);
            h.length = 0;
            e++;
            ...
            for (t = 0; t < q; t++) {
                var r = p[b.charAt(e)],
                    v = r >> 5 ? -1 : 1,
                    r = (r & 31) << 6 | p[b.charAt(e + 1)];
                3 == s && (r = r << 6 | p[b.charAt(e + 2)]);
                r = v * r / 10;
                t % 2 ? k = r += k : m = r += m;
                h[t] = r;
                e += s
...
(function () {
    ...
    a.updateContext = function (b) {
        var a, d = this.mask;
        d && (d.graphics && !d.graphics.isEmpty()) && (a = d.getMatrix(d._matrix), b.transform(a.a, a.b, a.c, a.d, a.tx, a.ty), d.graphics.drawAsPath(b), b.clip(),
            a.invert(), b.transform(a.a, a.b, a.c, a.d, a.tx, a.ty));
    ...
    a.cache = function (b,
        ...
        this.cacheCanvas.width = Math.ceil(d * e);
        this.cacheCanvas.height = Math.ceil(c * e);
    ...
    a.setTransform = function (b, a, d, c, e, f, h, k, m) {
        this.x = b || 0;
        this.y = a || 0;
    ...
    a.cloneProps = function (b) {
        ...
        b.compositeOperation = this.compositeOperation;
        this.cacheCanvas && (b.cacheCanvas = this.cacheCanvas.cloneNode(!0), b.cacheCanvas.getContext("2d").putImageData(this.cacheCanvas.getContext("2d").getImageData(0,
    ...
    a._applyFilters = function () {
        if (this.filters && 0 != this.filters.length && this.cacheCanvas) for (var b = this.filters.length, a = this.cacheCanvas.getContext("2d"), d = this.cacheCanvas.width, c = this.cacheCanvas.height, e = 0; e < b; e++) this.filters[e].applyFilter(a, 0, 0, d, c)
    };
    a._hasMouseHandler = function (b) {
        var a = this._listeners;
        return !!(b & 1 && (this.onPress || this.onClick || this.onDoubleClick || a && (this.hasEventListener("mousedown") || this.hasEventListener("click") || this.hasEventListener("dblclick"))) || b & 2 && (this.onMouseOver ||
            this.onMouseOut || this.cursor || a && (this.hasEventListener("mouseover") || this.hasEventListener("mouseout"))))
    };
...
(function () {
    ...
    a.addChildAt = function (b, a) {
        ...
        if (2 < d) {
            for (var e = 0; e < d - 1; e++) this.addChildAt(arguments[e], c + e);
    ...
    a.removeChildAt = function (b) {
        ...
        if (1 < a) {
            ...
            for (var e = !0, c = 0; c < a; c++) e = e && this.removeChildAt(d[c]);
            return e
        }
    ...
    a.swapChildren = function (b, a) {
        for (var d = this.children, c, e, f = 0, h = d.length; f < h && !(d[f] == b && (c = f), d[f] == a && (e = f), null != c && null != e) ; f++);
        f != h && (d[c] = a, d[e] = b)
    };
    a.setChildIndex = function (b, a) {
        ...
        if (!(b.parent != this || 0 > a || a >= c)) {
            for (var e = 0; e < c && d[e] != b; e++);
            e == c || e == a || (d.splice(e, 1), a < e && a--, d.splice(a, 0, b))
        }
    };
    ...
    a.clone = function (b) {
        var a = new c;
        this.cloneProps(a);
        if (b) for (var d = a.children = [], j = 0, e = this.children.length; j < e; j++) {
            var f = this.children[j].clone(b);
            f.parent = a;
            d.push(f)
        }
        return a
    ...
    a._getObjectsUnderPoint = function (b, a, d, j) {
        var e = createjs.DisplayObject._hitTestContext,
            ...
            h = this._matrix,
            k = this._hasMouseHandler(j);
        if (!this.hitArea && (this.cacheCanvas && k) && (this.getConcatenatedMatrix(h), e.setTransform(h.a, h.b, h.c, h.d, h.tx - b, h.ty - a), e.globalAlpha = h.alpha, this.draw(e),
            this._testHit(e))) return f.width = 0, f.width = 1, this;
        for (var m = this.children.length - 1; 0 <= m; m--) {
            var p = this.children[m],
                q = p.hitArea;
            if (p.visible && !(!q && !p.isVisible() || j && !p.mouseEnabled)) {
                var s = j && p._hasMouseHandler(j);
                if (p instanceof c && (!q || !s)) if (k) {
                    if (p = p._getObjectsUnderPoint(b, a)) return this
                } else {
                    if (p = p._getObjectsUnderPoint(b, a, d, j), !d && p) return p
                } else if (!j || k || s) if (p.getConcatenatedMatrix(h), q && (h.appendTransform(q.x, q.y, q.scaleX, q.scaleY, q.rotation, q.skewX, q.skewY, q.regX, q.regY), h.alpha = q.alpha),
                    e.globalAlpha = h.alpha, e.setTransform(h.a, h.b, h.c, h.d, h.tx - b, h.ty - a), (q || p).draw(e), this._testHit(e)) {
                    f.width = 0;
                    f.width = 1;
                    if (k) return this;
                    if (d) d.push(p);
...
(function () {
    ...
    a.initialize = function (b) {
        ...
        this.canvas = "string" ==
            typeof b ? document.getElementById(b) : b;
        this._pointerData = {};
    ...
    a.toDataURL = function (b, a) {
        ...
        var d = this.canvas.getContext("2d"),
            ...
            f;
        if (b) {
            f = d.getImageData(0, 0, c, e);
            var h = d.globalCompositeOperation;
            d.globalCompositeOperation = "destination-over";
            d.fillStyle = b;
            d.fillRect(0, 0, c, e)
        }
        var k = this.canvas.toDataURL(a);
        b && (d.clearRect(0, 0, c, e), d.putImageData(f, 0, 0), d.globalCompositeOperation = h);
        return k
    };
    ...
    a._handlePointerMove = function (b, a, d, c) {
        if (this.canvas) {
            ...
            if (f || e.inBounds || this.mouseMoveOutside) {
                if (this.onMouseMove || this.hasEventListener("stagemousemove")) d = new createjs.MouseEvent("stagemousemove", e.x, e.y, this, a, b, b == this._primaryPointerID, e.rawX, e.rawY), this.onMouseMove && this.onMouseMove(d), this.dispatchEvent(d);
                if ((c = e.event) && (c.onMouseMove || c.hasEventListener("mousemove"))) d = new createjs.MouseEvent("mousemove", e.x, e.y, c.target, a, b, b == this._primaryPointerID, e.rawX, e.rawY), c.onMouseMove && c.onMouseMove(d), c.dispatchEvent(d, c.target)
            }
        }
    };
    a._updatePointerPosition = function (b, a, d) {
        ...
        (c.inBounds = 0 <= a && 0 <= d && a <= e - 1 && d <= f - 1) ? (c.x = a, c.y = d) : this.mouseMoveOutside &&
            (c.x = 0 > a ? 0 : a > e - 1 ? e - 1 : a, c.y = 0 > d ? 0 : d > f - 1 ? f - 1 : d);
        c.rawX = a;
        c.rawY = d;
    ...
    a._handlePointerUp = function (b, a, d) {
        var c = this._getPointerData(b),
            e;
        if (this.onMouseMove ||
            this.hasEventListener("stagemouseup")) e = new createjs.MouseEvent("stagemouseup", c.x, c.y, this, a, b, b == this._primaryPointerID, c.rawX, c.rawY), this.onMouseUp && this.onMouseUp(e), this.dispatchEvent(e);
        var f = c.event;
        if (f && (f.onMouseUp || f.hasEventListener("mouseup"))) e = new createjs.MouseEvent("mouseup", c.x, c.y, f.target, a, b, b == this._primaryPointerID, c.rawX, c.rawY), f.onMouseUp && f.onMouseUp(e), f.dispatchEvent(e, f.target);
        if ((f = c.target) && (f.onClick || f.hasEventListener("click")) && this._getObjectsUnderPoint(c.x,
            c.y, null, !0, this._mouseOverIntervalID ? 3 : 1) == f) e = new createjs.MouseEvent("click", c.x, c.y, f, a, b, b == this._primaryPointerID, c.rawX, c.rawY), f.onClick && f.onClick(e), f.dispatchEvent(e);
    ...
    a._handlePointerDown = function (b, a, d, c) {
        ...
        if (this.onMouseDown || this.hasEventListener("stagemousedown")) d =
                new createjs.MouseEvent("stagemousedown", e.x, e.y, this, a, b, b == this._primaryPointerID, e.rawX, e.rawY), this.onMouseDown && this.onMouseDown(d), this.dispatchEvent(d);
        if (c = this._getObjectsUnderPoint(e.x, e.y, null, this._mouseOverIntervalID ? 3 : 1)) if (e.target = c, c.onPress || c.hasEventListener("mousedown")) if (d = new createjs.MouseEvent("mousedown", e.x, e.y, c, a, b, b == this._primaryPointerID, e.rawX, e.rawY), c.onPress && c.onPress(d), c.dispatchEvent(d), d.onMouseMove || d.onMouseUp || d.hasEventListener("mousemove") || d.hasEventListener("mouseup")) e.event =
                        d
    };
    ...
    a._handleDoubleClick = function (b) {
        ...
        if (d && (d.onDoubleClick || d.hasEventListener("dblclick"))) evt =
                new createjs.MouseEvent("dblclick", a.x, a.y, d, b, -1, !0, a.rawX, a.rawY), d.onDoubleClick && d.onDoubleClick(evt), d.dispatchEvent(evt)
    };
    createjs.Stage = c
})();
this.createjs = this.createjs || {};
...
(function () {
    ...
    a._normalizeFrame = function () {
        ...
        if (b) if (c = b.frames.length, this.currentAnimationFrame >= c) {
            var e = b.next;
            this._dispatchAnimationEnd(b, a, d, e, c - 1) || (e ? this._goto(e) : (this.paused = !0, this.currentAnimationFrame = b.frames.length - 1, this.currentFrame =
                b.frames[this.currentAnimationFrame]))
    ...
    a._dispatchAnimationEnd = function (b, a, d, c, e) {
        var f = b ? b.name : null;
        this.onAnimationEnd && this.onAnimationEnd(this, f, c);
        this.dispatchEvent({
            type: "animationend",
            name: f,
            next: c
        });
        !d && this.paused && (this.currentAnimationFrame = e);
    ...
    a._goto = function (b) {
        if (isNaN(b)) {
            ...
            a && (this.currentAnimationFrame = 0, this._animation = a, this.currentAnimation = b, this._normalizeFrame())
                ...
                this._animation = null, this.currentFrame = b
    };
...
(function () {
    ...
    a._drawText = function (b) {
        ...
        for (var d = String(this.text).split(/(?:\r\n|\r|\n)/), c = this.lineHeight || this.getMeasuredLineHeight(), e = 0, f = 0, h = d.length; f <
            h; f++) {
            var k = b.measureText(d[f]).width;
            if (null == this.lineWidth || k < this.lineWidth) a && this._drawTextLine(b, d[f], e * c);
            else {
                for (var k = d[f].split(/(\s)/), m = k[0], p = 1, q = k.length; p < q; p += 2) b.measureText(m + k[p] + k[p + 1]).width > this.lineWidth ? (a && this._drawTextLine(b, m, e * c), e++, m = k[p + 1]) : m += k[p] + k[p + 1];
                a && this._drawTextLine(b, m, e * c)
            }
            e++
        }
        return e
...
(function () {
    ...
    c._flip = function (a,
        b, g, d) {
        for (var j = a._images, e = c._workingCanvas, f = c._workingContext, h = j.length / b, k = 0; k < h; k++) {
            var m = j[k];
            m.__tmp = k;
            e.width = 0;
            e.width = m.width;
            e.height = m.height;
            f.setTransform(g ? -1 : 1, 0, 0, d ? -1 : 1, g ? m.width : 0, d ? m.height : 0);
            f.drawImage(m, 0, 0);
            var p = new Image;
        ...
        for (k = 0; k < e; k++) {
            ...
            g && (q.x = p.width - q.x - q.width, s.regX = q.width -
                m.regX);
            d && (q.y = p.height - q.y - q.height, s.regY = q.height - m.regY);
            f.push(s)
        }
        g = "_" + (g ? "h" : "") + (d ? "v" : "");
        d = a._animations;
        ...
        for (k = 0; k < j; k++) {
            ...
            f = m.frames;
            m = 0;
            for (p = f.length; m < p; m++) h.frames.push(f[m] + e * b);
            a[h.name] = h;
            d.push(h.name)
        }
    };
...
(function () {
    ...
    a.addFrame = function (b, a, d, j, e, f) {
    ...
    a.addMovieClip = function (b, a, d) {
        ...
        !e && b.getBounds && (e = b.getBounds());
        if (!e && !j) return null;
        a = this._frames.length;
        for (var f = b.timeline.duration, h = 0; h < f; h++) this.addFrame(b, j && j[h] ? j[h] : e, d, function (b) {
        ...
        if (b.length) {
            ...
            for (k = b.length; h < k; h++) {
                d = b[h].label;
                for (var j = a + (h == k - 1 ? f : b[h + 1].index), e = [], m = a + b[h].index; m < j; m++) e.push(m);
                this.addAnimation(d, e, !0)
            }
        }
    };
    a.build = function () {
    ...
    a._startBuild = function () {
        ...
        if (d[d.length -
            1].height + 2 * b > this.maxHeight) throw c.ERR_DIMENSIONS;
        for (var j = 0, e = 0, f = 0; d.length;) {
            var h = this._fillRow(d, j, f, a, b);
            h.w > e && (e = h.w);
            j += h.h;
            if (!h.h || !d.length) {
                var k = createjs.createCanvas ? createjs.createCanvas() : document.createElement("canvas");
                k.width = this._getSize(e, this.maxWidth);
                k.height = this._getSize(j, this.maxHeight);
    ...
    a._fillRow = function (b, a, d, j, e) {
        var f = this.maxWidth,
            h = this.maxHeight;
        a += e;
        for (var h = h - a, k = e, m = 0, p = b.length - 1; 0 <= p; p--) {
            var q = b[p],
                s = this._scale * q.scale,
                t = q.sourceRect,
                u = q.source,
                r = Math.floor(s * t.x - e),
                v = Math.floor(s * t.y - e),
                w = Math.ceil(s * t.height + 2 * e),
                t = Math.ceil(s * t.width + 2 * e);
            if (t > f) throw c.ERR_DIMENSIONS;
            w > h || k + t > f || (q.img = d, q.rect = new createjs.Rectangle(k, a, t, w), m = m || w, b.splice(p, 1), j[q.index] = [k, a, t, w, d, Math.round(-r + s * u.regX - e), Math.round(-v + s * u.regY - e)], k += t)
        }
        return {
            w: k,
            h: m
    ...
    a._drawNext = function () {
        ...
        e.save();
        e.beginPath();
        e.rect(d.x, d.y, d.width, d.height);
        e.clip();
        e.translate(Math.ceil(d.x - c.x * a), Math.ceil(d.y - c.y * a));
        e.scale(a, a);
        b.source.draw(e);
        e.restore();
...
(function () {
    ...
    a.draw = function () {
        if (null != this.htmlElement) {
            ...
            if (d.tx != b.tx || d.ty != b.ty || d.a != b.a || d.b != b.b || d.c != b.c || d.d != b.d) a.transform = a.WebkitTransform = a.OTransform = a.msTransform = ["matrix(" + b.a, b.b, b.c, b.d, b.tx + 0.5 | 0, (b.ty + 0.5 | 0) + ")"].join(), a.MozTransform = ["matrix(" + b.a, b.b, b.c, b.d, (b.tx + 0.5 | 0) + "px", (b.ty +
                        0.5 | 0) + "px)"].join(), this._oldMtx = b.clone();
            return !0
        }
    };
    a.cache = function () { };
...
(function () {
    ...
    c._IOS_handleEvent = function (a, b) {
        if (a) {
            a.__touch.preventDefault &&
                b.preventDefault && b.preventDefault();
            for (var c = b.changedTouches, d = b.type, j = 0, e = c.length; j < e; j++) {
                var f = c[j],
                    h = f.identifier;
                f.target == a.canvas && ("touchstart" == d ? this._handleStart(a, h, b, f.pageX, f.pageY) : "touchmove" == d ? this._handleMove(a, h, b, f.pageX, f.pageY) : ("touchend" == d || "touchcancel" == d) && this._handleEnd(a, h, b))
            }
        }
    };
    ...
    c._handleStart = function (a, b, c, d, j) {
        ...
        if (e.multitouch || !e.count) {
            var f = e.pointers;
            f[b] || (f[b] = !0, e.count++, a._handlePointerDown(b, c, d, j))
        }
    };
    c._handleMove = function (a, b, c, d, j) {
        a.__touch.pointers[b] &&
            a._handlePointerMove(b, c, d, j)
    };
...
(function () {
    ...
    c.version = "2013.02.12";
    c.buildDate = "Tue, 12 Feb 2013 21:34:28 GMT"
})();
this.createjs = this.createjs || {};
(function () {
    var c = function (b, a, d) {
    ...
    c.removeTweens = function (b) {
        if (b.tweenjs_count) {
            for (var a = c._tweens, d = a.length - 1; 0 <= d; d--) a[d]._target ==
                    b && (a[d]._paused = !0, a.splice(d, 1));
            b.tweenjs_count = 0
        }
    };
    ...
    c.installPlugin = function (b, a) {
        ...
        for (var j = 0, e = a.length, f = c._plugins; j < e; j++) {
            var h = a[j];
            if (f[h]) {
                for (var k = f[h], m = 0, p = k.length; m < p && !(d < k[m].priority) ; m++);
                f[h].splice(m, 0, b)
            } else f[h] = [b]
        }
    };
    c._register = function (b, a) {
        ...
        a ? (d && (d.tweenjs_count = d.tweenjs_count ? d.tweenjs_count + 1 : 1), c._tweens.push(b)) : (d && d.tweenjs_count--,
            d = c._tweens.indexOf(b), -1 != d && c._tweens.splice(d, 1))
    };
    a.addEventListener = null;
    a.removeEventListener = null;
    a.removeAllEventListeners = null;
    ...
    a.setPosition = function (b, a) {
        ...
        0 != a && 0 < this._actions.length &&
            (this._useTicks ? this._runActions(d, d) : 1 == a && d < e ? (e != this.duration && this._runActions(e, this.duration), this._runActions(0, d, !0)) : this._runActions(e, d));
        c && this.setPaused(!0);
    ...
    a._updateTargetProps = function (b, a) {
        ...
        for (n in this._initQueueProps) {
            if (null == (e = d[n])) d[n] = e = this._initQueueProps[n];
            if (null == (f = j[n])) j[n] = f = e;
            e = e == f || 0 == a || 1 == a || "number" != typeof e ? 1 == a ? f : e : e + (f - e) * a;
            var h = !1;
            if (f = c._plugins[n]) for (var k = 0, m = f.length; k < m; k++) {
                var p = f[k].tween(this, n, e, d, j, a, !!b && d == j, !b);
                p == c.IGNORE ? h = !0 : e = p
            }
            h || (this._target[n] = e)
        }
    ...
    a._runActions = function (a, c, d) {
        var j = a,
            ...
            k = 1;
        a > c && (j = c, e = a, f = h, h = k = -1);
        for (;
        (f += k) != h;) {
            c = this._actions[f];
            var m = c.t;
            (m == e || m > j && m < e || d && m == a) && c.f.apply(c.o, c.p)
        }
    };
    a._appendQueueProps = function (a) {
        ...
        for (h in a) {
            if (void 0 === this._initQueueProps[h]) {
                ...
                if (g = c._plugins[h]) {
                    j = 0;
                    for (e = g.length; j < e; j++) d = g[j].init(this, h, d)
                }
                this._initQueueProps[h] = void 0 === d ? null : d
            } else d = this._curQueueProps[h]; if (g = c._plugins[h]) {
                f = f || {};
                j = 0;
                for (e = g.length; j < e; j++) g[j].step && g[j].step(this, h, d, a[h], f)
            }
            this._curQueueProps[h] = a[h]
        }
        f &&
            this._appendQueueProps(f);
...
(function () {
    ...
    a.setPosition = function (a, c) {
        ...
        this.position = this._prevPos = d;
        for (var e = 0, f = this._tweens.length; e < f; e++) if (this._tweens[e].setPosition(d, c), d != this._prevPos) return !1;
        j && this.setPaused(!0);
        this.onChange && this.onChange(this);
        return j
    };
    ...
    a.updateDuration = function () {
        for (var a = this.duration = 0, c = this._tweens.length; a < c; a++) tween = this._tweens[a], tween.duration > this.duration && (this.duration = tween.duration)
    };
    a.tick = function (a) {
...
(function () {
    ...
    c.step = function (a, b, g, d, j) {
        ...
        if (6 <= g && 0 == (g - 2) % 4) {
            ...
            d._length = 0;
            for (b = 2; b < g; b += 4) {
                for (var f = a[b - 2], h = a[b - 1], k = a[b + 0], m = a[b + 1], p = a[b + 2], q = a[b + 3], s = f, t = h, u, r, v = 0, w = [], y = 1; 10 >= y; y++) {
                    r = y / 10;
                    var x = 1 - r;
                    u = x * x * f + 2 * x * r * k + r * r * p;
                    r = x * x * h + 2 * x * r * m + r * r * q;
                    v += w[w.push(Math.sqrt((e = u - s) * e + (e = r - t) * e)) - 1];
                    s = u;
                    t = r
                }
                d._segments.push(v);
    ...
    c.tween = function (a, b, g, d, j, e, f) {
        j = j.guide;
        if (void 0 == j || j === d.guide) return g;
        j.lastRatio != e && (c.calc(j, (j.end - j.start) * (f ? j.end : e) + j.start, a.target), j.orient && (a.target.rotation += d.rotation || 0), j.lastRatio = e);
    ...
    c.calc = function (a, b, g) {
        ...
        var d = a._segments,
            ...
            e = a._length * b,
            f = d.length - 2;
        for (b = 0; e > d[b] && b < f;) e -= d[b], b += 2;
        for (var d =
            d[b + 1], h = 0, f = d.length - 1; e > d[h] && h < f;) e -= d[h], h++;
        e = h / ++f + e / (f * d[h]);
        b = 2 * b + 2;
        f = 1 - e;
        g.x = f * f * j[b - 2] + 2 * f * e * j[b + 0] + e * e * j[b + 2];
        g.y = f * f * j[b - 1] + 2 * f * e * j[b + 1] + e * e * j[b + 3];
        a.orient && (g.rotation = 57.2957795 * Math.atan2((j[b + 1] - j[b - 1]) * f + (j[b + 3] - j[b + 1]) * e, (j[b + 0] - j[b - 2]) * f + (j[b + 2] - j[b + 0]) * e));
        return g
    };
    createjs.MotionGuidePlugin = c
})();
(function () {
    var c = this.createjs = this.createjs || {}, c = c.TweenJS = c.TweenJS || {};
    c.version = "2013.02.12";
    c.buildDate = "Tue, 12 Feb 2013 21:34:28 GMT"
})();
this.createjs = this.createjs || {};
(function () {
    ...
    c.sendLoadComplete = function (a) {
        if (c.preloadHash[a]) for (var b = 0, g = c.preloadHash[a].length; b < g; b++) {
            ...
            c.preloadHash[a][b] = !0;
            c.onLoadComplete && c.onLoadComplete(f);
            c.dispatchEvent(f)
        }
    };
    ...
    c.initLoad = function (a, b, g, f) {
        a = c.registerSound(a, g, f, !1);
        return null == a ? !1 : a
    };
    c.registerSound = function (b, g, e, f) {
        if (!c.initializeDefaultPlugins()) return !1;
        b instanceof Object && (b = b.src, g = b.id, e = b.data);
        var h = c.parsePath(b, "sound", g, e);
        if (null == h) return !1;
        null != g && (c.idHash[g] = h.src);
        var k = null;
        null != e && (isNaN(e.channels) ? isNaN(e) || (k = parseInt(e)) : k = parseInt(e.channels));
        var m = c.activePlugin.register(h.src, k);
        null != m && (null != m.numChannels && (k = m.numChannels), a.create(h.src, k), null == e || !isNaN(e) ? e = h.data = k || a.maxPerChannel() :
            e.channels = h.data.channels = k || a.maxPerChannel(), null != m.tag ? h.tag = m.tag : m.src && (h.src = m.src), null != m.completeHandler && (h.completeHandler = m.completeHandler), h.type = m.type);
        !1 != f && (c.preloadHash[h.src] || (c.preloadHash[h.src] = []), c.preloadHash[h.src].push({
            src: b,
            id: g,
            data: e
        }), 1 == c.preloadHash[h.src].length && c.activePlugin.preload(h.src, m));
        return h
    };
    ...
    c.play = function (a, b, g, f, h, k, m) {
        a = c.createInstance(a);
        c.playInstance(a,
            b, g, f, h, k, m) || a.playFailed();
        return a
    };
    c.createInstance = function (b) {
        if (!c.initializeDefaultPlugins()) return c.defaultSoundInstance;
        var g = c.parsePath(b, "sound");
        b = g ? c.getSrcById(g.src) : c.getSrcById(b);
        var g = b.lastIndexOf("."),
            e = b.slice(g + 1); -1 != g && -1 < c.SUPPORTED_EXTENSIONS.indexOf(e) ? (a.create(b), b = c.activePlugin.create(b)) : b = c.defaultSoundInstance;
        b.uniqueId = c.lastId++;
        return b
    };
    c.setVolume = function (a) {
        ...
        c.masterVolume = a;
        if (!this.activePlugin || !this.activePlugin.setVolume || !this.activePlugin.setVolume(a)) for (var b = this.instances, g = 0, f = b.length; g < f; g++) b[g].setMasterVolume(a)
    };
    ...
    c.setMute = function (a) {
        ...
        this.masterMute = a;
        if (!this.activePlugin || !this.activePlugin.setMute || !this.activePlugin.setMute(a)) for (var b =
                this.instances, c = 0, g = b.length; c < g; c++) b[c].setMasterMute(a);
        return !0
    };
    c.getMute = function () {
    ...
    c.playInstance = function (a, b, g, f, h, k, m) {
        b = b || c.defaultInterruptBehavior;
        null == g && (g = 0);
        null == f && (f = a.getPosition());
        null == h && (h = 0);
        null == k && (k = a.getVolume());
        null == m && (m = a.getPan());
        if (0 == g) {
            if (!c.beginPlaying(a, b, f, h, k, m)) return !1
        } else g = setTimeout(function () {
            c.beginPlaying(a, b, f, h, k, m)
        }, g), a.delayTimeoutId =
                g;
        this.instances.push(a);
        return !0
    };
    c.beginPlaying = function (b, c, g, f, h, k) {
        return !a.add(b, c) ? !1 : !b.beginPlaying(g, f, h, k) ? (b = this.instances.indexOf(b), -1 < b && this.instances.splice(b, 1), !1) : !0
    };
    ...
    var g = a.prototype = {
        ...
        getSlot: function (a) {
            for (var b, g, f = 0, h = this.max; f < h; f++) {
                b = this.get(f);
                if (null == b) return !0;
                if (!(a == c.INTERRUPT_NONE && b.playState != c.PLAY_FINISHED)) if (0 == f) g = b;
                else if (b.playState == c.PLAY_FINISHED || b == c.PLAY_INTERRUPTED || b == c.PLAY_FAILED) g = b;
...
(function () {
    ...
    c.generateCapabilities = function () {
        if (null == c.capabilities) {
            ...
            for (var b = createjs.Sound.SUPPORTED_EXTENSIONS, j = createjs.Sound.EXTENSION_MAP, e = 0, f = b.length; e < f; e++) {
                var h = b[e],
                    k = j[h] || h;
    ...
    a.prototype = {
        ...
        play: function (a, b, c, e, f, h) {
            this.cleanUp();
            createjs.Sound.playInstance(this, a, b, c, e, f, h)
        },
        beginPlaying: function (a, b, c, e) {
    ...
    b.prototype = {
        ...
        handleLoad: function () {
            c.context.decodeAudioData(this.request.response,
...
(function () {
    ...
    c.generateCapabilities = function () {
        if (null == c.capabilities) {
            ...
            for (var b = createjs.Sound.SUPPORTED_EXTENSIONS, g = createjs.Sound.EXTENSION_MAP, f = 0, h = b.length; f < h; f++) {
                var k = b[f],
                    m = g[k] || k;
                c.capabilities[k] = "no" != a.canPlayType("audio/" + k) && "" != a.canPlayType("audio/" + k) || "no" != a.canPlayType("audio/" + m) && "" != a.canPlayType("audio/" + m)
            }
        }
    };
    c.prototype = {
        ...
        register: function (a,
            ...
            for (var c = g.get(a), f, h = b || this.defaultNumChannels, k = 0; k < h; k++) f = this.createTag(a), c.add(f);
            return {
                tag: f,
                numChannels: h
            }
    ...
    a.prototype = {
        ...
        play: function (a, b, c, g, h, k) {
            this.cleanUp();
            createjs.Sound.playInstance(this, a, b, c,
                g, h, k)
        },
        beginPlaying: function (a, b, c) {
            if (null == window.createjs) return -1;
            var f = this.tag = g.getInstance(this.src);
            if (null == f) return this.playFailed(), -1;
            this.duration = 1E3 * this.tag.duration;
            f.addEventListener(createjs.HTMLAudioPlugin.AUDIO_ENDED, this.endedHandler, !1);
            this.offset = a;
            this.volume = c;
            this.updateVolume();
            this.remainingLoops = b;
            4 !== f.readyState ? (f.addEventListener(createjs.HTMLAudioPlugin.AUDIO_READY, this.readyHandler, !1), f.addEventListener(createjs.HTMLAudioPlugin.AUDIO_STALLED, this.stalledHandler, !1), f.load()) : this.handleSoundReady(null);
...
(function () {
    ...
    c.prototype = {};
    var a = c.prototype;
    c.FILE_PATTERN = /(\w+:\/{2})?((?:\w+\.){2}\w+)?(\/?[\S]+\/|\/)?([\w\-%\.]+)(?:\.)(\w+)?(\?\S+)?/i;
    a.loaded = !1;
    a.canceled = !1;
    a.progress = 0;
    a._item = null;
...
(function () {
    ...
    a.remove = function (a) {
        ...
        if (b) {
            for (; b.length;) {
                for (var c = b.pop(), e = this.getResult(c), f = this._loadQueue.length - 1; 0 <= f; f--) if (h = this._loadQueue[f].getItem(), h.id == c || h.src == c) {
                    this._loadQueue.splice(f, 1)[0].cancel();
                    break
                }
                for (f = this._loadQueueBackup.length - 1; 0 <=
                    f; f--) if (h = this._loadQueueBackup[f].getItem(), h.id == c || h.src == c) {
                        this._loadQueueBackup.splice(f, 1)[0].cancel();
                        break
                    }
                if (e) delete this._loadItemsById[e.id], delete this._loadItemsBySrc[e.src], this._disposeItem(e);
                else for (var f = this._currentLoads.length - 1; 0 <= f; f--) {
                    var h = this._currentLoads[f].getItem();
                    if (h.id == c || h.src == c) {
    ...
    a.installPlugin = function (a) {
        if (!(null == a || null == a.getPreloadHandlers)) {
            ...
            if (null != a.types) for (var b = 0, c = a.types.length; b < c; b++) this._typeCallbacks[a.types[b]] =
                        a.callback;
    ...
    a.loadFile = function (a, b) {
        null == a ? this._sendError({
            text: "PRELOAD_NO_FILE"
        }) : (this._addItem(a), !1 !== b && this.setPaused(!1))
    };
    a.loadManifest = function (a, b) {
        var c = null;
        ...
        for (var e = 0, f = c.length; e < f; e++) this._addItem(c[e]);
    ...
    a._createLoadItem = function (a) {
        ...
        if (a = this._typeCallbacks[b.type] || this._extensionCallbacks[b.ext]) {
            ...
            if (!1 === a) return null;
            !0 !== a && (null != a.src && (b.src = a.src), null != a.id && (b.id = a.id), null !=
                a.tag && a.tag.load instanceof Function && (b.tag = a.tag), null != a.completeHandler && (b.completeHandler = a.completeHandler));
    ...
    a._processFinishedLoad = function (a) {
        this._numItemsLoaded++;
        this._updateProgress();
    ...
    a._updateProgress = function () {
        ...
        if (0 < b) {
            for (var c = 0, e = 0, f = this._currentLoads.length; e < f; e++) c += this._currentLoads[e].progress;
    ...
    a._createTag = function (a) {
        ...
        switch (a) {
            ...
            case createjs.LoadQueue.SVG:
                return this.useXHR ? b = document.createElement("svg") :
                    (b = document.createElement("object"), b.type = "image/svg+xml"), b
        }
        return null
    };
...
(function () {
    ...
    a.load = function () {
        ...
        if (a.type == createjs.LoadQueue.SVG || a.type == createjs.LoadQueue.JAVASCRIPT || a.type == createjs.LoadQueue.CSS) (document.body || document.getElementsByTagName("body")[0]).appendChild(c);
        null != c.load && c.load()
    };
    ...
    a._handleLoad = function () {
        if (!this._isCanceled()) {
            var a = this.getItem(),
                c = a.tag;
            this.loaded || this.isAudio && 4 !== c.readyState || (this.loaded = !0, a.type == createjs.LoadQueue.SVG && (document.body || document.getElementsByTagName("body")[0]).removeChild(c), this._clean(), this._sendComplete())
        }
    };
    a._clean = function () {
        clearTimeout(this._loadTimeout);
    ...
    a._generateTag = function () {
        ...
        switch (this._item.type) {
            ...
            case createjs.LoadQueue.SVG:
                ...
                this._rawResponse = this._response;
                a.appendChild(c.documentElement);
                this._response = a;
                break;
            case createjs.LoadQueue.JSON:
                try {
